<script lang="ts">
  ...
  async function load(): Promise<void> {
    ...
    error = "";
    try {
      const res = await fetch("/api/hugs/disk?files=1");
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      report = data.report as DirReport;
      orphans = (data.orphans ?? []) as string[];
    } catch (cause) {
        ...
        <tbody>
          {#each report.files ?? [] as f (f.name)}
            <tr class="border-b last:border-0 hover:bg-muted/50">
              <td class="px-3 py-1.5 font-mono text-xs">{f.name}</td>
