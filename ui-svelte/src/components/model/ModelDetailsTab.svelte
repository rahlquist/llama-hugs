<script lang="ts">
  ...
  import { capabilityLabels, formatContextLength } from "../../lib/capabilities";
  import * as Card from "$lib/components/ui/card/index.js";
  import Tag from "../Tag.svelte";
  import HugsTagsEditor from "./HugsTagsEditor.svelte";
...
<Card.Root class="shrink-0 gap-0 overflow-hidden py-0">
  ...
  <Card.Content class="space-y-3 p-3">
      ...
      {:else}
        <div class="flex flex-wrap gap-1.5">
          ...
          {#if (model.context_length ?? 0) > 0}
            <Tag class="bg-sky-500/15 text-sky-700 dark:text-sky-300">
    ...
    <div>
      <p class="text-muted-foreground mb-1.5 text-xs font-medium uppercase tracking-wide">Custom tags</p>
      <HugsTagsEditor modelId={model.id} />
    </div>
  </Card.Content>
</Card.Root>
